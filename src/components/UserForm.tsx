...
function UserFrom (errorMessage: string){
  return(<>
    ...
    <input id='name' name = "name" placeholder='名前を入力してください' className={errorMessage.includes("Name")? "error":""}/><br/>
  </>);
}
...
function EmailFrom (errorMessage: string){
  return(<>
    ...
    <input id='email' name = "email" placeholder='emailを入力してください' className={errorMessage.includes("Email")? "error":""}/><br/>
  </>);
}
...
function AgeFrom (selectedAgeOptions:  Array<string>, errorMessage: string){
  return (<>
    ...
    <select name="selectedAge" className={(stringHelpers.includesAny(errorMessage,"年齢","Age"))? "error":""}>
